    ...
    methods: {
        initMask() {
            try {
                ...
                this.maskCore.setValue(this.$refs.input.value);
                ...
                this.$emit('inputted', this.$refs.input.value, '');
            }
        },
        getValue() {
        ...
        keyUp(e) {
            if (e.keyCode === 9) { // Prevent change selection for Tab in
                return;
            }
            this.updateToCoreState();
        ...
        updateToCoreState() {
            ...
            if (this.$refs.input.value !== this.maskCore.getValue()) {
                ...
                this.$emit('inputted', this.$refs.input.value, this.maskCore.getRawValue());
            }
            this.$refs.input.selectionStart = this.maskCore.selection.start;
        ...
        change(e) {
            e.preventDefault();
            const length = Object.keys(this.maskCore.pattern._editableIndices).length;
            const text = String(e.target.value).split('').slice(-length);
            this.maskCore.setValue(text.join(''));
            this.updateToCoreState();
        },
    },
};